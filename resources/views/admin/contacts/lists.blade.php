                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped font-14" id="datatable-basic">
                            ...
                            <tr>
                                <th class="text-left">{{ trans('oraclepopupsignin/main.user_name') }}</th>
                                <th class="text-left">{{ trans('oraclepopupsignin/main.email') }}</th>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if($contact->status =='replied')
                                            <span class="text-success">{{ trans('oraclepopupsignin/main.replied') }}</span>
                                        @else
                                    ...
                                    <td width="100">
                                        @can('admin_contacts_reply')
                                            <a href="/oraclepopupsignin/contacts/{{ $contact->id }}/reply" class="btn-transparent btn-sm text-primary" data-toggle="tooltip" data-placement="top" title="{{ trans('oraclepopupsignin/main.reply') }}">
                                                <i class="fa fa-reply"></i>
                                            </a>
                                        @endcan

                                        @can('admin_contacts_delete')
                                            @include('admin.includes.delete_button',['url' => '/oraclepopupsignin/contacts/'. $contact->id.'/delete','btnClass' => 'btn-sm'])
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="contactMessageLabel">{{ trans('oraclepopupsignin/main.contacts_message') }}</h5>
