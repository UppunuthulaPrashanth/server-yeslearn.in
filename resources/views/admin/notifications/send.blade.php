        ...
        <div class="section-body">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="post" action="/oraclepopupsignin/notifications/{{ !empty($notification) ? $notification->id .'/update' : 'store' }}" class="form-horizontal form-bordered mt-4">
                        {{ csrf_field() }}
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="type" id="typeSelect" class="form-control @error('type') is-invalid @enderror">
                                        ...
                                        @foreach(\App\Models\Notification::$notificationsType as $type)
                                            <option value="{{ $type }}" @if(!empty($notification) and $notification->type == $type) selected @endif>{{ trans('oraclepopupsignin/main.notification_'.$type) }}</option>
                                        @endforeach
                                    </select>
                                    <div class="invalid-feedback">@error('type') {{ $message }} @enderror</div>
                                    <div class="text-muted text-small mt-1">{{ trans('oraclepopupsignin/main.new_notification_hint') }}</div>
                                </div>
                                ...
                                <div class="form-group @if(!$errors->has('group_id') and empty($notification->group)) d-none @endif" id="groupSelect">
                                    <label class="input-label d-block">{{ trans('oraclepopupsignin/main.group') }}</label>
