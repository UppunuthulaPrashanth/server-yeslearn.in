                ...
                <div class="card-body">
                    <form class="mb-0">
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="input-label">{{trans('oraclepopupsignin/main.sender')}}</label>
                                    <select name="sender" data-plugin-selectTwo class="form-control populate">
                                        <option value="">Select Sender</option>
                                        <option value="admin" @if(request()->get('sender') == 'admin') selected @endif>{{trans('oraclepopupsignin/main.admin_role')}}</option>
                                        <option value="organizations" @if(request()->get('sender') == 'organizations') selected @endif>{{trans('oraclepopupsignin/main.organizations')}}</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="input-label">{{trans('oraclepopupsignin/main.types')}}</label>
                                    <select name="type" data-plugin-selectTwo class="form-control populate">
                                        <option value="">{{trans('oraclepopupsignin/main.all_types')}}</option>
                ...
                <div class="card-header">
                    @can('admin_noticeboards_send')
                        <div class="text-right">
                            <a href="/oraclepopupsignin/noticeboards/send" class="btn btn-primary">{{trans('oraclepopupsignin/main.send_noticeboard')}}</a>
                        </div>
                    @endcan
                </div>
                            ...
                            @foreach($noticeboards as $noticeboard)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <button type="button" data-item-id="{{ $noticeboard->id }}" class="js-show-description btn btn-outline-primary">{{ trans('oraclepopupsignin/main.show') }}</button>
                                        <input type="hidden" value="{{ nl2br($noticeboard->message) }}">
                                    </td>
                                    <td class="text-center">{{ trans('oraclepopupsignin/main.notification_'.$noticeboard->type) }}</td>

                                    <td class="text-center">{{ dateTimeFormat($noticeboard->created_at,'j M Y | H:i') }}</td>

                                    <td width="100">
                                        @can('admin_noticeboards_edit')
                                            <a href="/oraclepopupsignin/noticeboards/{{ $noticeboard->id }}/edit" class="btn-transparent  text-primary" data-toggle="tooltip" data-placement="top" title="{{ trans('oraclepopupsignin/main.edit') }}">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                        @endcan

                                        @can('admin_notifications_delete')
                                            @include('admin.includes.delete_button',['url' => '/oraclepopupsignin/noticeboards/'. $noticeboard->id.'/delete','btnClass' => ''])
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
