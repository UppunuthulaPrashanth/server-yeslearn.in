...
@section('content')
    <section class="section">
        ...
        <div class="section-body">
            <section class="card">
                <div class="card-body">
                    <form action="{{ !empty($region) ? '/oraclepopupsignin/regions/'.$region->id.'/update' : '/oraclepopupsignin/regions/store' }}" method="post">
                        {{ csrf_field() }}
                        ...
                        <div class="row">
                            <div class="col-12 col-lg-6">
                                ...
                                <div id="countrySelectBox" class="form-group {{ !empty($countries) ? '' : 'd-none' }}">
                                    ...
                                    <select name="country_id" class="form-control search-region-select2 @error('country_id') is-invalid @enderror" data-type="{{ \App\Models\Region::$country }}" data-placeholder="{{ trans('oraclepopupsignin/main.search') }} {{ trans('update.countries') }}">

                                        @if(!empty($countries))
                                            <option value="">{{ trans('oraclepopupsignin/main.select') }} {{ trans('update.country') }}</option>
                                ...
                                <div id="provinceSelectBox" class="form-group {{ ((!empty($region) and ($region->type == \App\Models\Region::$city or $region->type == \App\Models\Region::$district)) or (!empty(request()->get('type')) and (request()->get('type') == \App\Models\Region::$city or request()->get('type') == \App\Models\Region::$district))) ? '' : 'd-none'}}">
                                    ...
                                    <select name="province_id" {{ empty($provinces) ? 'disabled' : '' }} class="form-control @error('province_id') is-invalid @enderror">
                                        <option value="">{{ trans('oraclepopupsignin/main.select') }} {{ trans('update.province') }}</option>
                                ...
                                <div id="citySelectBox" class="form-group {{ ((!empty($region) and $region->type == \App\Models\Region::$district) or (!empty(request()->get('type')) and request()->get('type') == \App\Models\Region::$district)) ? '' : 'd-none'}}">
                                    ...
                                    <select name="city_id" {{ empty($cities) ? 'disabled' : '' }} class="form-control @error('city_id') is-invalid @enderror">
                                        <option value="">{{ trans('oraclepopupsignin/main.select') }} {{ trans('update.city') }}</option>
                                ...
                                <div class="form-group">
                                    <label for="" class="input-label">{{ trans('oraclepopupsignin/main.title') }}</label>
                                    <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ !empty($region) ? $region->title : '' }}" placeholder="{{ trans('oraclepopupsignin/main.title') }}">
                                    @error('title')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                        ...
                        <button type="submit" class="btn btn-success mt-4">{{ trans('oraclepopupsignin/main.save') }}</button>
                    </form>
                </div>
            </section>
        </div>
