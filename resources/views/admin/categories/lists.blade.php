...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('oraclepopupsignin/main.categories') }}</h1>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @can('admin_categories_edit')
                                                    <a href="/oraclepopupsignin/categories/{{ $category->id }}/edit"
                                                       class="btn-transparent btn-sm text-primary">
                                                        <i class="fa fa-edit"></i>
                                                    </a>
                                                @endcan
                                                @can('admin_categories_delete')
                                                    @include('admin.includes.delete_button',['url' => '/oraclepopupsignin/categories/'.$category->id.'/delete'])
                                                @endcan
                                            </td>
                                        </tr>
                                    @endforeach
