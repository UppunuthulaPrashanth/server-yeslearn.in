...
@section('content')
        ...
        <div class="row">
                ...
                <div class="card card-statistic-1">
                    ...
                    <div class="card-wrap">
                        <div class="card-header">
                            <h4>{{trans('oraclepopupsignin/main.total_appointments')}}</h4>
                        </div>
                        <div class="card-body">
                            {{ $totalAppointments }}
                        </div>
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                <div class="card card-statistic-1">
                    ...
                    <div class="card-wrap">
                        <div class="card-header">
                            <h4>{{trans('oraclepopupsignin/main.open_appointments')}}</h4>
                        </div>
                        <div class="card-body">
                            {{ $openAppointments }}
                        </div>
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                <div class="card card-statistic-1">
                    ...
                    <div class="card-wrap">
                        <div class="card-header">
                            <h4>{{trans('oraclepopupsignin/main.finished_appointments')}}</h4>
                        </div>
                        <div class="card-body">
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                <div class="card card-statistic-1">
                    ...
                    <div class="card-wrap">
                        <div class="card-header">
                            <h4>{{trans('oraclepopupsignin/main.total_reservatores')}}</h4>
                        </div>
                        <div class="card-body">
                            {{ $totalConsultants }}
                        </div>
                ...
                <div class="card-body">
                    <form method="get" class="mb-0">
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="sort" data-plugin-selectTwo class="form-control populate">
                                        ...
                                        <option value="has_discount" @if(request()->get('sort') == 'has_discount') selected @endif>{{trans('oraclepopupsignin/main.discounted_appointments')}}</option>
                                        <option value="free" @if(request()->get('sort') == 'free') selected @endif>{{trans('oraclepopupsignin/main.free_appointments')}}</option>
                                        <option value="amount_asc" @if(request()->get('sort') == 'amount_asc') selected @endif>{{trans('oraclepopupsignin/main.cost_ascending')}}</option>
                                        <option value="amount_desc" @if(request()->get('sort') == 'amount_desc') selected @endif>{{trans('oraclepopupsignin/main.cost_descending')}}</option>
                                        <option value="date_asc" @if(request()->get('sort') == 'date_asc') selected @endif>{{trans('oraclepopupsignin/main.date_ascending')}}</option>
                                        <option value="date_desc" @if(request()->get('sort') == 'date_desc') selected @endif>{{trans('oraclepopupsignin/main.date_descending')}}</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label class="input-label">{{trans('oraclepopupsignin/main.consultant')}}</label>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label class="input-label">{{trans('oraclepopupsignin/main.reservatore')}}</label>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped text-center font-14">
                            ...
                            <tr>
                                <th class="text-left">{{trans('oraclepopupsignin/main.consultant')}}</th>
                                <th class="text-left">{{trans('oraclepopupsignin/main.reservatore')}}</th>
                                <th class="text-center">{{ trans('update.meeting_type') }}</th>
                                <th class="text-center">{{trans('oraclepopupsignin/main.cost')}}</th>
                                <th class="text-center">{{trans('oraclepopupsignin/main.date')}}</th>
                                <th class="text-center">{{trans('oraclepopupsignin/main.time')}}</th>
                                <th class="text-center">{{ trans('update.students_count') }}</th>
                                <th class="text-center">{{trans('oraclepopupsignin/main.status')}}</th>
                                <th class="text-center">{{trans('oraclepopupsignin/main.actions')}}</th>
                            </tr>
                                    ...
                                    <td class="text-center" width="50">
                                        ...
                                        @can('admin_appointments_join')
                                            @if(!empty($appointment->link) and $appointment->status == \App\Models\ReserveMeeting::$open)
                                                <button type="button" data-reserve-id="{{ $appointment->id }}" class="js-show-join-modal btn-transparent text-primary" data-toggle="tooltip" data-placement="top" title="{{trans('oraclepopupsignin/main.join_link')}}"><i class="fa fa-link" aria-hidden="true"></i></button>
                                            @endif
                                        @endcan

                                        @can('admin_appointments_send_reminder')
                                            <button type="button" data-reserve-id="{{ $appointment->id }}" class="js-send-reminder btn-transparent text-primary" data-toggle="tooltip" data-placement="top" title="{{trans('oraclepopupsignin/main.send_appointment_reminder')}}"><i class="fa fa-bell" aria-hidden="true"></i></button>
                                        @endcan

                                        @can('admin_appointments_cancel')
                                            @if($appointment->status != \App\Models\ReserveMeeting::$canceled)
                                                @include('admin.includes.delete_button',['url' => '/oraclepopupsignin/appointments/'.$appointment->id.'/cancel'])
                                            @endif
                                        @endcan
                                    </td>
                                </tr>
        ...
        <div class="card-body">
           <div class="section-title ml-0 mt-0 mb-3"> <h5>{{trans('oraclepopupsignin/main.hints')}}</h5> </div>
            <div class="row">
                <div class="col-md-4">
                    <div class="media-body">
                        <div class="text-primary mt-0 mb-1 font-weight-bold">{{trans('oraclepopupsignin/main.appointments_hint_title_1')}}</div>
                        <div class=" text-small font-600-bold">{{trans('oraclepopupsignin/main.appointments_hint_description_1')}}</div>
                    </div>
                </div>

                <div class="col-md-4">
                    <div class="media-body">
                        <div class="text-primary mt-0 mb-1 font-weight-bold">{{trans('oraclepopupsignin/main.appointments_hint_title_2')}}</div>
                        <div class=" text-small font-600-bold">{{trans('oraclepopupsignin/main.appointments_hint_description_2')}}</div>
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="media-body">
                        <div class="text-primary mt-0 mb-1 font-weight-bold">{{trans('oraclepopupsignin/main.appointments_hint_title_3')}}</div>
                        <div class="text-small font-600-bold">{{trans('oraclepopupsignin/main.appointments_hint_description_3')}}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="contactMessageLabel">{{trans('oraclepopupsignin/main.join_appointment')}}</h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-12 col-md-8">
                            <div class="form-group">
                                <label class="input-label">{{trans('oraclepopupsignin/main.url')}}</label>
                        ...
                        <div class="col-12 col-md-4">
                            <div class="form-group">
                                <label class="input-label">{{trans('oraclepopupsignin/main.password')}}</label>
                ...
                <div class="modal-footer">
                    <a href="" target="_blank" class="js-join-btn btn btn-primary">{{trans('oraclepopupsignin/main.join')}}</a>
    ...
    <div class="modal fade" id="sendReminderModal" tabindex="-1" aria-labelledby="contactMessageLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="contactMessageLabel">{{trans('oraclepopupsignin/main.send_reminder')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="">
                        <strong>{{trans('oraclepopupsignin/main.consultant')}}:</strong>
                        <span class="js-consultant"></span>
                    </div>

                    <div class="mt-2">
                        <strong>{{trans('oraclepopupsignin/main.reservatore')}}:</strong>
                        <span class="js-reservatore"></span>
                    </div>

                    <div class="mt-2">
                        <strong>{{trans('oraclepopupsignin/main.remind_title')}}:</strong>
                        <span class="js-title"></span>
                    </div>

                    <div class="mt-2">
                        <strong>{{trans('oraclepopupsignin/main.remind_message')}}:</strong>
                        <span class="js-message"></span>
                    </div>
                </div>

                <div class="modal-footer">
                    <a href="" class="js-send-reminder-btn btn btn-primary">{{trans('oraclepopupsignin/main.send')}}</a>
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ trans('oraclepopupsignin/main.close') }}</button>
                </div>
            </div>
        </div>
