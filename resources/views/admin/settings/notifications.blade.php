...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('oraclepopupsignin/main.notifications') }}</h1>
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item">{{ trans('oraclepopupsignin/main.notifications') }}</div>
            </div>
        </div>
                        ...
                        <div class="card-body">
                            ...
                            <ul class="nav nav-pills" id="myTab3" role="tablist">
                                @foreach(\App\Models\NotificationTemplate::$notificationTemplateAssignSetting as $section => $v)
                                    <li class="nav-item">
                                        <a class="nav-link {{ $loop->iteration == 1 ? ' active' : '' }}" id="{{ $section }}-tab" data-toggle="tab" href="#{{ $section }}" role="tab" aria-controls="{{ $section }}" aria-selected="true">{{ trans('oraclepopupsignin/main.notification_'.$section) }}</a>
                                    </li>
                                @endforeach
                            </ul>
                                    ...
                                    <div class="tab-pane mt-3 fade {{ $loop->iteration == 1 ? ' show active' : '' }}" id="{{ $tab }}" role="tabpanel" aria-labelledby="{{ $tab }}-tab">
                                        <div class="row">
                                            <div class="col-12 col-md-6">
                                                <form action="/oraclepopupsignin/settings/notifications/store" method="post">
                                                    {{ csrf_field() }}

                                                    @foreach($items as $item)
                                                        <div class="form-group">
                                                            <label class="input-label">{{ trans('oraclepopupsignin/main.notification_'.$item) }}</label>
