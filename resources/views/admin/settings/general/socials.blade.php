<div class="tab-pane mt-3 fade @if(!empty($social)) show active @endif" id="socials" role="tabpanel" aria-labelledby="socials-tab">
    <div class="row">
        <div class="col-12 col-md-8 col-lg-6">
            <form action="/oraclepopupsignin/settings/socials/store" method="post">
                {{ csrf_field() }}
                ...
                <button type="submit" class="btn btn-success mt-3">{{ trans('oraclepopupsignin/main.submit') }}</button>
            </form>
        </div>
    </div>
    ...
    <div class="table-responsive mt-5">
        <table class="table table-striped font-14">
            <tr>
                <th>{{ trans('oraclepopupsignin/main.icon') }}</th>
                ...
                @if(!empty($itemValue) and is_array($itemValue))
                    @foreach($itemValue as $key => $val)
                        <tr>
                            ...
                            <td>
                                <a href="/oraclepopupsignin/settings/socials/{{ $key }}/edit" class="btn-transparent text-primary" data-toggle="tooltip" data-placement="top" title="{{ trans('oraclepopupsignin/main.edit') }}">
                                    <i class="fa fa-edit"></i>
                                </a>

                                @include('admin.includes.delete_button',['url' => '/oraclepopupsignin/settings/socials/'. $key .'/delete','btnClass' => ''])
                            </td>
                        </tr>
                    @endforeach
                @endif
