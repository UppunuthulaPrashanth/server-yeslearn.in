...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('oraclepopupsignin/main.footer') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/oraclepopupsignin/">{{ trans('oraclepopupsignin/pages/dashboard.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ trans('oraclepopupsignin/pages/setting.footer') }}</div>
            </div>
        </div>
                        ...
                        <div class="card-body">
                            ...
                            <ul class="nav nav-pills" id="myTab3" role="tablist">
                                @foreach($columns as $column)
                                    <li class="nav-item">
                                        <a class="nav-link {{ $loop->iteration == 1 ? 'active' : '' }}" id="{{ $column }}-tab" data-toggle="tab" href="#{{ $column }}" role="tab" aria-controls="{{ $column }}" aria-selected="true">{{ trans('oraclepopupsignin/pages/setting.footer_'.$column) }}</a>
                                    </li>
                                @endforeach
                            </ul>
                                    ...
                                    <div class="tab-pane mt-3 fade {{ $loop->iteration == 1 ? 'show active' : '' }}" id="{{ $column }}" role="tabpanel" aria-labelledby="{{ $column }}-tab">
                                        <div class="row">
                                            <div class="col-12 col-md-6">
                                                <form action="/oraclepopupsignin/additional_page/footer/store" method="post">
                                                    {{ csrf_field() }}
                                                    ...
                                                    <div class="form-group">
                                                        <label>{{ trans('oraclepopupsignin/main.content') }}</label>
