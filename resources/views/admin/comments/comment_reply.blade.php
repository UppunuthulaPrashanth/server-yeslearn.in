...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('oraclepopupsignin/main.reply_comment') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/oraclepopupsignin/">{{trans('oraclepopupsignin/main.dashboard')}}</a>
                </div>
                <div class="breadcrumb-item">{{ trans('oraclepopupsignin/main.reply_comment') }}</div>
            </div>
        </div>
                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header flex-column align-items-start">
                            <h4>{{ trans('oraclepopupsignin/main.main_comment') }}</h4>
                            ...
                            @if(!empty($comment->replies) and $comment->replies->count() > 0)
                                <div class="mt-1 w-100">
                                    <h4>{{ trans('oraclepopupsignin/main.reply_list') }}</h4>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span class="text-{{ ($reply->status == 'pending') ? 'warning' : 'success' }}">
                                                            {{ ($reply->status == 'pending') ? trans('oraclepopupsignin/main.pending') : trans('oraclepopupsignin/main.published') }}
                                                        </span>
                                                    </td>

                                                    <td>
                                                        <span class="text-{{ (empty($reply->reply_id)) ? 'info' : 'warning' }}">
                                                            {{ (empty($reply->reply_id)) ? trans('oraclepopupsignin/main.main_comment') : trans('oraclepopupsignin/main.replied') }}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td>
                                                        ...
                                                        @can('admin_'. $itemRelation .'_comments_status')
                                                            <a href="/oraclepopupsignin/comments/{{ $page }}/{{ $reply->id }}/toggle" class="btn-transparent text-primary">
                                                        ...
                                                        @can('admin_'. $itemRelation .'_comments_edit')
                                                            <a href="/oraclepopupsignin/comments/{{ $page }}/{{ $reply->id }}/edit" class="btn-transparent text-primary" data-toggle="tooltip" data-placement="top" title="{{ trans('oraclepopupsignin/main.edit') }}">
                                                                <i class="fa fa-edit"></i>
                                                            </a>
                                                        @endcan

                                                        @can('admin_'. $itemRelation .'_comments_delete')
                                                            @include('admin.includes.delete_button',['url' => '/oraclepopupsignin/comments/'. $page .'/'.$reply->id.'/delete','btnClass' => 'btn-sm mt-2'])
                                                        @endcan
                                                    </td>
                                                </tr>
                                            @endforeach
                        ...
                        <div class="card-body ">
                            <form action="/oraclepopupsignin/comments/{{ $page }}/{{ $comment->id }}/reply" method="post">
                                {{ csrf_field() }}

                                <div class="form-group mt-15">
                                    <label class="input-label">{{ trans('oraclepopupsignin/main.reply_comment') }}</label>
                                ...
                                <button type="submit" class="mt-3 btn btn-primary">{{ trans('oraclepopupsignin/main.save_change') }}</button>
                            </form>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="contactMessageLabel">{{ trans('oraclepopupsignin/main.comment') }}</h5>
