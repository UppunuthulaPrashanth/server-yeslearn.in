...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('oraclepopupsignin/main.documents') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/oraclepopupsignin/">{{trans('oraclepopupsignin/main.dashboard')}}</a>
                </div>
                <div class="breadcrumb-item">{{ trans('oraclepopupsignin/main.documents') }}</div>
            </div>
        </div>
        <div class="section-filters">
            <section class="card">
                <div class="card-body">
                    <div class="mt-3">
                        <form action="/oraclepopupsignin/financial/documents" method="get" class="row mb-0">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label
                                        class="input-label d-block">{{ trans('/oraclepopupsignin/main.user') }}</label>
                                    <select name="user[]" multiple="" class="form-control search-user-select2"
                                            data-placeholder="{{ trans('/oraclepopupsignin/main.search_user_or_instructor') }}">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label class="input-label d-block">{{ trans('oraclepopupsignin/main.class') }}</label>
                                    <select name="webinar" class="form-control search-webinar-select2"
                                            data-placeholder="{{ trans('oraclepopupsignin/main.search_webinar') }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="type" class="form-control">
                                        ...
                                        <option value="addiction"
                                                @if(request()->get('type',null) == 'addiction') selected="selected" @endif>{{ trans('oraclepopupsignin/main.addiction') }}</option>
                                        <option value="deduction"
                                                @if(request()->get('type',null) == 'deduction') selected="selected" @endif>{{ trans('oraclepopupsignin/main.deduction') }}</option>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label class="input-label d-block">{{ trans('oraclepopupsignin/main.type_account') }}</label>
                                    <select name="type_account" class="form-control">
                                        ...
                                        <option value="asset"
                                                @if(request()->get('type_account',null) == 'asset') selected="selected" @endif>{{ trans('oraclepopupsignin/main.asset') }}</option>
                                        <option value="income"
                                                @if(request()->get('type_account',null) == 'income') selected="selected" @endif>{{ trans('oraclepopupsignin/main.income') }}</option>
        ...
        <div class="section-body">
            ...
            <div class="d-flex align-items-center justify-content-between">
                ...
                {{--<a href="/oraclepopupsignin/offline_payments/excel" class="btn btn-danger">{{ trans('oraclepopupsignin/main.export_xls') }}</a>--}}
            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped font-14">
                                    <tr>
                                        ...
                                        <th class="text-center">{{ trans('oraclepopupsignin/main.tax') }}</th>
                                        <th class="text-center">{{ trans('oraclepopupsignin/main.system') }}</th>
                                        <th>{{ trans('oraclepopupsignin/main.amount') }}</th>
                                        <th>{{ trans('oraclepopupsignin/main.type') }}</th>
                                        <th>{{ trans('oraclepopupsignin/main.creator') }}</th>
                                        <th>{{ trans('oraclepopupsignin/main.type_account') }}</th>
                                            ...
                                            <tr>
                                                <td class="text-left">
                                                    <div class="text-left">
                                                        ...
                                                        @elseif(!empty($document->meeting_time_id))
                                                            <span class="d-block font-weight-bold">{{ trans('oraclepopupsignin/main.item_purchased') }}</span>
                                                            <a href=""
                                                               target="_blank" class="font-12">#{{ $document->meeting_time_id }} {{ trans('oraclepopupsignin/main.meeting') }}</a>
                                                        ...
                                                        @elseif($document->store_type == \App\Models\Accounting::$storeManual)
                                                            <span class="d-block font-weight-bold">{{ trans('oraclepopupsignin/main.manual_document') }}</span>
                                                        @else
                                                            <span class="d-block font-weight-bold">{{ trans('oraclepopupsignin/main.automatic_document') }}</span>
                                                        @endif
                                                    </div>
                                                </td>
                                                ...
                                                <td class="text-left">
                                                    @if(!empty($document->user))
                                                        <a href="/oraclepopupsignin/users/{{ $document->user_id }}/edit" target="_blank"
                                                ...
                                                <td>
                                                    @if($document->creator_id)
                                                        <span>{{ trans('oraclepopupsignin/main.admin') }}</span>
                                                    @else
                                                        <span>{{ trans('oraclepopupsignin/main.automatic') }}</span>
                                                    @endif
                                                </td>
                                                ...
                                                <td>
                                                    @can('admin_documents_print')
                                                        <a href="/oraclepopupsignin/financial/documents/{{ $document->id }}/print" class="btn-sm fa fa-print"></a>
                                                    @endcan
                                                </td>
                                            </tr>
                                        @endforeach
