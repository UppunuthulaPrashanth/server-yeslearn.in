    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item">{{ trans('oraclepopupsignin/main.offline_payments') }}</div>
            </div>
        </div>
                    ...
                    <form class="mb-0">
                        ...
                        <div class="row">
                                ...
                                <div class="col-md-2">
                                    <div class="form-group">
                                        ...
                                        <select name="status" data-plugin-selectTwo class="form-control populate">
                                            ...
                                            <option value="approved" @if(request()->get('status') == 'approved') selected @endif>{{trans('oraclepopupsignin/main.approved')}}</option>
                                            <option value="reject" @if(request()->get('status') == 'reject') selected @endif>{{trans('oraclepopupsignin/main.rejected')}}</option>
                                        </select>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="@if($pageType == 'requests') col-md-3 @else col-md-2 @endif">
                                <div class="form-group">
                                    <label class="input-label">{{trans('oraclepopupsignin/main.role')}}</label>
                                    <select name="role_id" data-plugin-selectTwo class="form-control populate">
                                        <option value="">{{trans('oraclepopupsignin/main.all_roles')}}</option>
                            ...
                            <div class="@if($pageType == 'requests') col-md-3 @else col-md-2 @endif">
                                <div class="form-group">
                                    <label class="input-label">{{trans('oraclepopupsignin/main.bank')}}</label>
                                    <select name="account_type" data-plugin-selectTwo class="form-control populate">
                                        <option value="">{{trans('oraclepopupsignin/main.all_banks')}}</option>
                            ...
                            <div class="@if($pageType == 'requests') col-md-3 @else col-md-2 @endif">
                                <div class="form-group">
                                    ...
                                    <select name="sort" data-plugin-selectTwo class="form-control populate">
                                        ...
                                        <option value="amount_asc" @if(request()->get('sort') == 'amount_asc') selected @endif>{{trans('oraclepopupsignin/main.amount_ascending')}}</option>
                                        <option value="amount_desc" @if(request()->get('sort') == 'amount_desc') selected @endif>{{trans('oraclepopupsignin/main.amount_descending')}}</option>
                                        <option value="pay_date_asc" @if(request()->get('sort') == 'pay_date_asc') selected @endif>{{trans('oraclepopupsignin/main.Transaction_time_ascending')}}</option>
                                        <option value="pay_date_desc" @if(request()->get('sort') == 'pay_date_desc') selected @endif>{{trans('oraclepopupsignin/main.Transaction_time_descending')}}</option>
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="card">
                        <div class="card-header">
                            @can('admin_offline_payments_export_excel')
                                <a href="/oraclepopupsignin/financial/offline_payments/excel?{{ http_build_query(request()->all()) }}" class="btn btn-primary">{{ trans('oraclepopupsignin/main.export_xls') }}</a>
                            @endcan
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped font-14">
                                    <tr>
                                        ...
                                        <th>{{trans('oraclepopupsignin/main.amount')}}</th>
                                        <th>{{trans('oraclepopupsignin/main.bank')}}</th>
                                        <th>{{trans('oraclepopupsignin/main.referral_code')}}</th>
                                        <th>{{trans('oraclepopupsignin/main.phone')}}</th>
                                        <th>{{trans('update.attachment')}}</th>
                                        <th width=180px>{{trans('oraclepopupsignin/main.transaction_time')}}</th>
                                        ...
                                        @if($pageType == 'requests')
                                            <th width="150px">{{trans('oraclepopupsignin/main.actions')}}</th>
                                        @endcan
                                    </tr>
                                                        ...
                                                        @if($offlinePayment->status == \App\Models\OfflinePayment::$waiting)
                                                            @can('admin_offline_payments_approved')
                                                                @include('admin.includes.delete_button',[
                                                                        'url' => '/oraclepopupsignin/financial/offline_payments/'. $offlinePayment->id .'/approved',
                                                            ...
                                                            @can('admin_offline_payments_reject')
                                                                @include('admin.includes.delete_button',[
                                                                        'url' => '/oraclepopupsignin/financial/offline_payments/'. $offlinePayment->id .'/reject',
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="media-body">
                        <div class="text-primary mt-0 mb-1 font-weight-bold">{{trans('oraclepopupsignin/main.offline_payment_hint_title_1')}}</div>
                        <div class=" text-small font-600-bold">{{trans('oraclepopupsignin/main.offline_payment_hint_description_1')}}</div>
                    </div>
                </div>

                <div class="col-md-6">
                    <div class="media-body">
                        <div class="text-primary mt-0 mb-1 font-weight-bold">{{trans('oraclepopupsignin/main.offline_payment_hint_title_2')}}</div>
                        <div class=" text-small font-600-bold">{{trans('oraclepopupsignin/main.offline_payment_hint_description_2')}}</div>
                    </div>
                </div>
            </div>
        </div>
