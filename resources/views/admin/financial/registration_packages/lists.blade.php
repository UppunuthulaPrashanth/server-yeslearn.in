                ...
                <div class="card card-statistic-1">
                    ...
                    <div class="card-wrap">
                        <div class="card-header">
                            <h4>{{ trans('oraclepopupsignin/main.total') }}</h4>
                        </div>
                        <div class="card-body">
                            {{ $totalPackages }}
                        </div>
                                    ...
                                    @foreach($packages as $package)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <span class="{{ $package->status == 'active' ? 'text-success' : 'text-danger' }}">{{ trans('oraclepopupsignin/main.'.$package->status) }}</span>
                                            </td>
                                            <td class="text-center">{{ dateTimeFormat($package->created_at, 'Y M j | H:i') }}</td>
                                            <td>
                                                @can('admin_registration_packages_edit')
                                                    <a href="/oraclepopupsignin/financial/registration-packages/{{ $package->id }}/edit" class="btn-sm" data-toggle="tooltip" data-placement="top" title="{{ trans('oraclepopupsignin/main.edit') }}">
                                                        <i class="fa fa-edit"></i>
                                                    </a>
                                                @endcan

                                                @can('admin_registration_packages_delete')
                                                    @include('admin.includes.delete_button',['url' => '/oraclepopupsignin/financial/registration-packages/'. $package->id.'/delete','btnClass' => 'btn-sm'])
                                                @endcan
                                            </td>
                                        </tr>
                                    @endforeach
