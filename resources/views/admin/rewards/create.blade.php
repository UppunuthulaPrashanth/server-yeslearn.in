                                    ...
                                    @if(!empty($rewards))
                                        @foreach($rewards as $reward)
                                            <tr>
                                                ...
                                                <td>{{ $reward->score }}</td>
                                                <td>{{ trans('oraclepopupsignin/main.'.$reward->status) }}</td>
                                                <td>{{ dateTimeFormat($reward->created_at,'j M Y') }}</td>
                                                <td>
                                                    @can('admin_rewards_items')
                                                        <button type="button" class="js-edit-reward btn-transparent btn-sm text-primary" data-id="{{ $reward->id }}" data-toggle="tooltip" data-placement="top" title="{{ trans('oraclepopupsignin/main.edit') }}">
                                                            <i class="fa fa-edit"></i>
                                                        </button>
                                                    @endcan

                                                    @can('admin_rewards_item_delete')
                                                        @include('admin.includes.delete_button',['url' => '/oraclepopupsignin/rewards/items/'.$reward->id.'/delete' , 'btnClass' => 'btn-sm'])
                                                    @endcan
                                                </td>
                                            </tr>
                                        @endforeach
                ...
                <div class="modal-body">
                    <form>
                        ...
                        <div class="custom-control custom-switch">
                            ...
                            <label class="custom-control-label" for="statusSwitch">{{ trans('oraclepopupsignin/main.active') }}</label>
                        </div>
                    </form>
                </div>

                <div class="modal-footer">
                    <button type="button" class="js-save-reward btn btn-primary">{{trans('oraclepopupsignin/main.save')}}</button>
