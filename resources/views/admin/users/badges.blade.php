...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ trans('oraclepopupsignin/main.badges') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="/oraclepopupsignin/">{{ trans('oraclepopupsignin/main.dashboard') }}</a>
                </div>
                <div class="breadcrumb-item active"><a href="/oraclepopupsignin/users">{{ trans('oraclepopupsignin/main.users') }}</a>
                </div>
                <div class="breadcrumb-item">{{ trans('oraclepopupsignin/main.badges') }}</div>
            </div>
        </div>

        <div class="section-body">
            <h2 class="section-title">{{ !empty($badge) ? trans('/oraclepopupsignin/main.edit') : trans('oraclepopupsignin/main.create') }}</h2>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            @if(empty($badge))
                                ...
                                <ul class="nav nav-pills" id="myTab3" role="tablist">
                                    @foreach(\App\Models\Badge::$badgeTypes as $type)
                                        <li class="nav-item">
                                            <a class="nav-link {{ $loop->iteration == 1 ? 'active' : '' }}" id="{{ $type }}-tab" data-toggle="tab" href="#{{ $type }}" role="tab" aria-controls="{{ $type }}" aria-selected="true">{{ trans('oraclepopupsignin/main.'.$type) }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                                        ...
                                        <div class="tab-pane mt-3 fade {{ $loop->iteration == 1 ? 'show active' : '' }}" id="{{ $type }}" role="tabpanel" aria-labelledby="{{ $type }}-tab">
                                            <div class="row">
                                                <div class="col-12 col-md-6">
                                                    <form action="/oraclepopupsignin/users/badges/store" method="post">
                                                        {{ csrf_field() }}
                                                        ...
                                                        <div class="form-group">
                                                            <label class="input-label">{{ trans('oraclepopupsignin/main.image') }}</label>
                                                        ...
                                                        <div class="form-group">
                                                            <label class=" control-label">{{ trans('oraclepopupsignin/main.condition') }}</label>

                                                            <div class="input-group">
                                                            <span class="input-group-prepend">
                                                                <span class="input-group-text">{{ trans('oraclepopupsignin/main.from') }}</span>
                                                            </span>
                                                                <input type="number" name="condition[from]" class="form-control @error('condition.from')  is-invalid @enderror">

                                                                <span class="input-group-append">
                                                                <span class="input-group-text">{{ trans('oraclepopupsignin/main.to') }}</span>
                                                            </span>
                                                                <input type="number" name="condition[to]" class="form-control @error('condition.from')  is-invalid @enderror">


                                                                <div class="input-group-append">
                                                                    <div class="input-group-text">{{ trans('oraclepopupsignin/main.condition_'.$type) }}</div>
                                                                </div>
                                                        ...
                                                        <div class="form-group">
                                                            <label>{{ trans('oraclepopupsignin/main.description') }}</label>
                                                        ...
                                                        <button type="submit" class="btn btn-success">{{ trans('oraclepopupsignin/main.submit') }}</button>
                                                    </form>
                                                </div>
                                            </div>
                                            ...
                                            <div class="table-responsive mt-5">
                                                <table class="table table-striped font-14">
                                                    <tr>
                                                        ...
                                                        <th>{{ trans('oraclepopupsignin/main.condition') }}</th>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    @can('admin_users_badges_edit')
                                                                        <a href="/oraclepopupsignin/users/badges/{{ $badge->id }}/edit" class="btn-sm" data-toggle="tooltip" data-placement="top" title="{{ trans('oraclepopupsignin/main.edit') }}">
                                                                            <i class="fa fa-edit"></i>
                                                                        </a>
                                                                    @endcan

                                                                    @can('admin_users_badges_delete')
                                                                        @include('admin.includes.delete_button',['url' => '/oraclepopupsignin/users/badges/'.$badge->id.'/delete' , 'btnClass' => 'btn-sm'])
                                                                    @endcan
                                                                </td>
                                                            </tr>
                                                        @endforeach
                            ...
                            @else
                                <div class="row">
                                    <div class="col-12 col-md-6">
                                        <form action="/oraclepopupsignin/users/badges/{{ $badge->id }}/update" method="post">
                                            {{ csrf_field() }}
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <div class="input-group-append">
                                                        <div class="input-group-text">{{ trans('oraclepopupsignin/main.condition_'.$badge->type) }}</div>
                                                    </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="media-body">
                        <div class="text-primary mt-0 mb-1 font-weight-bold">{{trans('oraclepopupsignin/main.badges_hint_title_1')}}</div>
                        <div class=" text-small font-600-bold">{{trans('oraclepopupsignin/main.badges_hint_description_1')}}</div>
                    </div>
                </div>

                <div class="col-md-6">
                    <div class="media-body">
                        <div class="text-primary mt-0 mb-1 font-weight-bold">{{trans('oraclepopupsignin/main.badges_hint_title_2')}}</div>
                        <div class=" text-small font-600-bold">{{trans('oraclepopupsignin/main.badges_hint_description_2')}}</div>
                    </div>
                </div>
            </div>
        </div>
