<div class="tab-pane mt-3 fade" id="badges" role="tabpanel" aria-labelledby="badges-tab">
    <div class="row">
        <div class="col-12 col-md-6">
            <form action="/oraclepopupsignin/users/{{ $user->id .'/badgesUpdate' }}" method="Post">
                {{ csrf_field() }}

                <div class="form-group">
                    <select name="badge_id" class="form-control @error('badge_id') is-invalid @enderror">
                        <option value="">{{ trans('oraclepopupsignin/main.select_badge') }}</option>
        ...
        <div class="col-12">
            <div class="mt-5">
                <h5>{{ trans('oraclepopupsignin/main.custom_badges') }}</h5>

                <div class="table-responsive mt-5">
                    <table class="table table-striped table-md">
                        <tr>
                            <th>{{ trans('oraclepopupsignin/main.title') }}</th>
                            <th>{{ trans('oraclepopupsignin/main.image') }}</th>
                            <th>{{ trans('oraclepopupsignin/main.condition') }}</th>
                            <th>{{ trans('oraclepopupsignin/main.description') }}</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @can('admin_users_edit')
                                            @include('admin.includes.delete_button',['url' => '/oraclepopupsignin/users/'.$user->id.'/deleteBadge/'.$customBadge->id , 'btnClass' => 'btn-sm'])
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
        ...
        <div class="col-12">
            <div class="mt-5">
                <h5>{{ trans('oraclepopupsignin/main.auto_badges') }}</h5>
