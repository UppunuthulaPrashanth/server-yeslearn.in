<div class="tab-pane mt-3 fade" id="financial" role="tabpanel" aria-labelledby="financial-tab">
    <div class="row">
        <div class="col-12 col-md-6">
            <form action="/oraclepopupsignin/users/{{ $user->id .'/financialUpdate' }}" method="Post">
                {{ csrf_field() }}
                ...
                @if(!$user->isUser())
                    <div class="form-group">
                        <label>{{ trans('oraclepopupsignin/main.user_commission') }} (%)</label>
                        <input type="text" name="commission"
                               class="form-control "
                               value="{{ !empty($user) ? $user->commission : old('commission') }}"
                               placeholder="{{ trans('oraclepopupsignin/main.user_commission_placeholder') }}"/>
                    </div>
                @endif
                ...
                <div class="form-group mb-0 d-flex">
                    ...
                    <label for="verifySwitch">{{ trans('oraclepopupsignin/main.financial_approval') }}</label>
                </div>
