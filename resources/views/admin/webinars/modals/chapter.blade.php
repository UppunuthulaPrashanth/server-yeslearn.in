...
<div class="d-none" id="webinarChapterModal">
    ...
    <form action="/oraclepopupsignin/chapters/store" method="post">
        ...
        <div class="form-group">
            <label class="input-label">{{ trans('oraclepopupsignin/main.type') }}</label>
            <select class="custom-select" name="type" required>
                @foreach(\App\Models\WebinarChapter::$chapterTypes as $chapterType)
                    <option value="{{ $chapterType }}">{{ trans('oraclepopupsignin/main.'.$chapterType) }}</option>
                @endforeach
            </select>
            <div class="invalid-feedback"></div>
        </div>
        ...
        <div class="js-chapter-status form-group mt-3">
            <div class="d-flex align-items-center justify-content-between">
                <label class="cursor-pointer input-label" for="chapterStatusSwitch_record">{{ trans('oraclepopupsignin/main.active') }}</label>
